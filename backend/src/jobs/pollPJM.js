...
const logger = require('../config/logger').forJob('pollPJM');
...
async function pollDemandFromEIA() {
  ...
  logger.info('Polling PJM demand via EIA (MIDA region)');
  ...
  try {
    ...
    logger.info(`PJM via EIA: received ${records.length} records`);
    ...
    for (const rec of records) {
      ...
      await upsertEnergyPrice(normalizeEnergyPrice({
        regionCode: 'PJM', timestamp: ts, pricePerMwh: null,
        priceType: 'real_time_hourly',
        pricingNode: `EIA_MIDA_${rec.type}`,
    ...
    try { await markHealthSuccess('PJM', elapsed); } catch (e) {
      logger.warn('Could not update PJM health', { error: e.message });
    }
    logger.info(`PJM demand: load=${demandMw}MW, netGen=${netGenMw}MW (${elapsed}ms)`);
    return { demandMw, netGenMw };

  } catch (err) {
    await safeMarkFailure('PJM', err.message);
    logger.error('PJM EIA poll failed', { error: err.message });
    throw err;
  }
}
...
async function pollFuelMixFromEIA() {
  const start = Date.now();
  logger.info('Polling PJM fuel mix via EIA');
  await sleep(600);
  ...
  try {
    ...
    for (const key of sortedKeys.slice(0, 1)) {
      ...
      const fuelMixRow = normalizeFuelMix('PJM', ts, fuels, 'EIA');
      await upsertFuelMix(fuelMixRow);
      const carbonRow = normalizeCarbonIntensity('PJM', ts, fuelMixRow, 'EIA');
      if (carbonRow) {
        await upsertCarbonIntensity(carbonRow);
      } else {
        logger.warn('Skipped carbon intensity — zero total generation', { region: 'PJM', timestamp: ts });
      }
      fuelCount++;
      logger.info(`PJM fuel mix: nuclear=${fuelMixRow.nuclear_pct?.toFixed(1)}%, gas=${fuelMixRow.natural_gas_pct?.toFixed(1)}%, renewables=${fuelMixRow.renewable_total_pct?.toFixed(1)}%`);
    }

    try { await markHealthSuccess('PJM', Date.now() - start); } catch (e) {
      logger.warn('Could not update PJM health', { error: e.message });
    }
    return { fuelCount };
    ...
    await safeMarkFailure('PJM', err.message);
    logger.error('PJM fuel mix poll failed', { error: err.message });
    throw err;
  }
}

async function run() {
  logger.info('=== PJM poll starting ===');
  const results = {};
  ...
  logger.info('=== PJM poll complete ===', results);
  return results;
}
