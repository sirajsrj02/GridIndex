...
const logger = require('../config/logger').forJob('pollISONE');
...
async function pollDemandFromEIA() {
  ...
  logger.info('Polling ISONE demand via EIA (NE region)');
  ...
  try {
    ...
    logger.info(`ISONE via EIA: received ${records.length} records`);
    ...
    for (const rec of records) {
      ...
      await upsertEnergyPrice(normalizeEnergyPrice({
        regionCode: 'ISONE', timestamp: ts, pricePerMwh: null,
        priceType: 'real_time_hourly',
        pricingNode: `EIA_NE_${rec.type}`,
    ...
    try { await markHealthSuccess('ISONE', elapsed); } catch (e) {
      logger.warn('Could not update ISONE health', { error: e.message });
    }
    logger.info(`ISONE demand: load=${demandMw}MW, netGen=${netGenMw}MW (${elapsed}ms)`);
    return { demandMw, netGenMw };

  } catch (err) {
    await safeMarkFailure('ISONE', err.message);
    logger.error('ISONE EIA poll failed', { error: err.message });
    throw err;
  }
}
...
async function pollFuelMixFromEIA() {
  const start = Date.now();
  logger.info('Polling ISONE fuel mix via EIA');
  await sleep(600);
  ...
  try {
    ...
    for (const key of sortedKeys.slice(0, 1)) {
      ...
      const fuelMixRow = normalizeFuelMix('ISONE', ts, fuels, 'EIA');
      await upsertFuelMix(fuelMixRow);
      const carbonRow = normalizeCarbonIntensity('ISONE', ts, fuelMixRow, 'EIA');
      if (carbonRow) {
        await upsertCarbonIntensity(carbonRow);
      } else {
        logger.warn('Skipped carbon intensity — zero total generation', { region: 'ISONE', timestamp: ts });
      }
      fuelCount++;
      logger.info(`ISONE fuel mix: nuclear=${fuelMixRow.nuclear_pct?.toFixed(1)}%, gas=${fuelMixRow.natural_gas_pct?.toFixed(1)}%, renewables=${fuelMixRow.renewable_total_pct?.toFixed(1)}%`);
    }

    try { await markHealthSuccess('ISONE', Date.now() - start); } catch (e) {
      logger.warn('Could not update ISONE health', { error: e.message });
    }
    return { fuelCount };
    ...
    await safeMarkFailure('ISONE', err.message);
    logger.error('ISONE fuel mix poll failed', { error: err.message });
    throw err;
  }
}

async function run() {
  logger.info('=== ISONE poll starting ===');
  const results = {};
  ...
  logger.info('=== ISONE poll complete ===', results);
  return results;
}
