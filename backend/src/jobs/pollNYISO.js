...
const logger = require('../config/logger').forJob('pollNYISO');
...
async function pollDemandFromEIA() {
  ...
  logger.info('Polling NYISO demand via EIA (NY region)');
  ...
  try {
    ...
    logger.info(`NYISO via EIA: received ${records.length} records`);
    ...
    for (const rec of records) {
      ...
      await upsertEnergyPrice(normalizeEnergyPrice({
        regionCode: 'NYISO', timestamp: ts, pricePerMwh: null,
        priceType: 'real_time_hourly',
        pricingNode: `EIA_NY_${rec.type}`,
    ...
    try { await markHealthSuccess('NYISO', elapsed); } catch (e) {
      logger.warn('Could not update NYISO health', { error: e.message });
    }
    logger.info(`NYISO demand: load=${demandMw}MW, netGen=${netGenMw}MW (${elapsed}ms)`);
    return { demandMw, netGenMw };

  } catch (err) {
    await safeMarkFailure('NYISO', err.message);
    logger.error('NYISO EIA poll failed', { error: err.message });
    throw err;
  }
}
...
async function pollFuelMixFromEIA() {
  const start = Date.now();
  logger.info('Polling NYISO fuel mix via EIA');
  await sleep(600);
  ...
  try {
    ...
    for (const key of sortedKeys.slice(0, 1)) {
      ...
      const fuelMixRow = normalizeFuelMix('NYISO', ts, fuels, 'EIA');
      await upsertFuelMix(fuelMixRow);
      const carbonRow = normalizeCarbonIntensity('NYISO', ts, fuelMixRow, 'EIA');
      if (carbonRow) {
        await upsertCarbonIntensity(carbonRow);
      } else {
        logger.warn('Skipped carbon intensity — zero total generation', { region: 'NYISO', timestamp: ts });
      }
      fuelCount++;
      logger.info(`NYISO fuel mix: nuclear=${fuelMixRow.nuclear_pct?.toFixed(1)}%, hydro=${fuelMixRow.hydro_pct?.toFixed(1)}%, renewables=${fuelMixRow.renewable_total_pct?.toFixed(1)}%`);
    }

    try { await markHealthSuccess('NYISO', Date.now() - start); } catch (e) {
      logger.warn('Could not update NYISO health', { error: e.message });
    }
    return { fuelCount };
    ...
    await safeMarkFailure('NYISO', err.message);
    logger.error('NYISO fuel mix poll failed', { error: err.message });
    throw err;
  }
}

async function run() {
  logger.info('=== NYISO poll starting ===');
  const results = {};
  ...
  logger.info('=== NYISO poll complete ===', results);
  return results;
}
